...
export default function OrderDetail({
  ...
  function handleDelete(data: CartData) {
    setCartData(cartData.filter((cart) => cart.book._id !== data.book._id));
    setSelected(selected.filter((cart) => cart.book._id !== data.book._id));
  }
                      ...
                      onCheckedChange={(v) =>
                        setSelected(
                          v
                            ? [...selected, data]
                            : selected.filter(
                                (cart) => data.book._id !== cart.book._id,
                              ),
                        )
                      }
                      checked={
                        !!selected.find(
                          (cart) => cart.book._id === data.book._id,
                        )
                      }
                    />
